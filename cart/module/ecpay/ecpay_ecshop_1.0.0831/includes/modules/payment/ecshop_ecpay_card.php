...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/ecshop_ecpay_card.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 描述對應的語言項 */
    $modules[$i]['desc'] = 'ecshop_ecpay_card_desc';
    ...
    $modules[$i]['config'] = array(
        array('name' => 'ecshop_ecpay_card_test_mode', 'type' => 'select', 'value' => 'Yes'),
        array('name' => 'ecshop_ecpay_card_account', 'type' => 'text', 'value' => '2000132'),
        array('name' => 'ecshop_ecpay_card_iv', 'type' => 'text', 'value' => 'v77hoKGq4kWxNNIS'),
        array('name' => 'ecshop_ecpay_card_key', 'type' => 'text', 'value' => '5294y06JbISpM5x9')
    );
    return;
}
 ...
 * 類
 */
class ecshop_ecpay_card extends AllInOne {
    ...
    function __construct() {
        parent::__construct();
        $this->ecshop_ecpay_card();
    }

    function ecshop_ecpay_card() {
    ...
    function get_code($order, $payment) {
        $isTestMode = ($payment['ecshop_ecpay_card_test_mode'] == 'Yes');
        ...
        $this->HashKey = trim($payment['ecshop_ecpay_card_key']);
        $this->HashIV = trim($payment['ecshop_ecpay_card_iv']);
        $this->MerchantID = trim($payment['ecshop_ecpay_card_account']);
     ...
     */
    function respond() {
        $arPayment = get_payment('ecshop_ecpay_card');
        $isTestMode = ($arPayment['ecshop_ecpay_card_test_mode'] == 'Yes');
        ...
        $this->HashKey = trim($arPayment['ecshop_ecpay_card_key']);
        $this->HashIV = trim($arPayment['ecshop_ecpay_card_iv']);
        ...
        try {
            ...
            if (sizeof($arFeedback) > 0) {
                ...
                $this->MerchantID = trim($arPayment['ecshop_ecpay_card_account']);
