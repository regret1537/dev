...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/ecshop_ecpay_atm.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 描述對應的語言項 */
    $modules[$i]['desc'] = 'ecshop_ecpay_atm_desc';
    ...
    $modules[$i]['config'] = array(
        array('name' => 'ecshop_ecpay_atm_test_mode', 'type' => 'select', 'value' => 'Yes'),
        array('name' => 'ecshop_ecpay_atm_account', 'type' => 'text', 'value' => '2000132'),
        array('name' => 'ecshop_ecpay_atm_iv', 'type' => 'text', 'value' => 'v77hoKGq4kWxNNIS'),
        array('name' => 'ecshop_ecpay_atm_key', 'type' => 'text', 'value' => '5294y06JbISpM5x9'),
    );
    return;
}
 ...
 * 類
 */
class ecshop_ecpay_atm extends AllInOne {
    ...
    function __construct() {
        parent::__construct();
        $this->ecshop_ecpay_atm();
    }

    function ecshop_ecpay_atm() {
    ...
    function get_code($order, $payment) {
        $isTestMode = ($payment['ecshop_ecpay_atm_test_mode'] == 'Yes');
        ...
        $this->HashKey = trim($payment['ecshop_ecpay_atm_key']);
        $this->HashIV = trim($payment['ecshop_ecpay_atm_iv']);
        $this->MerchantID = trim($payment['ecshop_ecpay_atm_account']);
     ...
     */
    function respond() {
        $arPayment = get_payment('ecshop_ecpay_atm');
        $isTestMode = ($arPayment['ecshop_ecpay_atm_test_mode'] == 'Yes');
        ...
        $this->HashKey = trim($arPayment['ecshop_ecpay_atm_key']);
        $this->HashIV = trim($arPayment['ecshop_ecpay_atm_iv']);
        ...
        try {
            ...
            if (sizeof($arFeedback) > 0) {
                ...
                $this->MerchantID = trim($arPayment['ecshop_ecpay_atm_account']);
