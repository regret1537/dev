...
$payment_lang = ROOT_PATH . 'languages/' . $GLOBALS['_CFG']['lang'] . '/payment/ecshop_allpay_tuu.php';
...
if (isset($set_modules) && $set_modules == TRUE) {
    ...
    /* 描述對應的語言項 */
    $modules[$i]['desc'] = 'ecshop_allpay_tuu_desc';
    ...
    $modules[$i]['config'] = array(
        array('name' => 'ecshop_allpay_tuu_test_mode', 'type' => 'select', 'value' => 'Yes'),
        array('name' => 'ecshop_allpay_tuu_account', 'type' => 'text', 'value' => '2000132'),
        array('name' => 'ecshop_allpay_tuu_iv', 'type' => 'text', 'value' => 'v77hoKGq4kWxNNIS'),
        array('name' => 'ecshop_allpay_tuu_key', 'type' => 'text', 'value' => '5294y06JbISpM5x9')
    );
    return;
}
 ...
 * 類
 */
class ecshop_allpay_tuu extends AllInOne {
    ...
    function __construct() {
        parent::__construct();
        $this->ecshop_allpay_tuu();
    }

    function ecshop_allpay_tuu() {
    ...
    function get_code($order, $payment) {
        $isTestMode = ($payment['ecshop_allpay_tuu_test_mode'] == 'Yes');
        ...
        $this->HashKey = trim($payment['ecshop_allpay_tuu_key']);
        $this->HashIV = trim($payment['ecshop_allpay_tuu_iv']);
        $this->MerchantID = trim($payment['ecshop_allpay_tuu_account']);
        ...
        $this->Send['ChoosePayment'] = PaymentMethod::TopUpUsed;
        $this->Send['Remark'] = '';
     ...
     */
    function respond() {
        $arPayment = get_payment('ecshop_allpay_tuu');
        $isTestMode = ($arPayment['ecshop_allpay_tuu_test_mode'] == 'Yes');
        ...
        $this->HashKey = trim($arPayment['ecshop_allpay_tuu_key']);
        $this->HashIV = trim($arPayment['ecshop_allpay_tuu_iv']);
        ...
        try {
            ...
            if (sizeof($arFeedback) > 0) {
                ...
                $this->MerchantID = trim($arPayment['ecshop_allpay_tuu_account']);
