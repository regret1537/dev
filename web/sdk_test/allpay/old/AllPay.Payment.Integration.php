...
class AllInOne {
    ...
    function QueryTradeInfo() {
        ...
        if (sizeof($arErrors) == 0) {
			...
			echo 'szCheckMacValue:'.$szCheckMacValue.'|szConfirmMacValue:'.$szConfirmMacValue.'<br />';# test
                if ($szCheckMacValue != strtoupper($szConfirmMacValue)) {
                    array_push($arErrors, 'CheckMacValue verify fail.');
                }
            }
    ...
    private function ServerPost($parameters) {
		...
		echo 'ServiceURL'.$this->ServiceURL.'<br />';# test
		echo 'parameters<pre>'.print_r($parameters, true).'</pre><br />';# test
        ...
        $rs = curl_exec($ch);
		echo 'rs<pre>'.print_r($rs, true).'</pre><br />';# test
